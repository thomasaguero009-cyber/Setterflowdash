...
async function findOrCreateDateRow(sheets, sheetName, fecha, opts) {
  ...
  await limpiarFormatoNumericoFilaNueva(sheets, sheetName, newRow, fechaCol);
  return newRow;
}

// Filas nuevas no heredan explícitamente formato de número en las columnas
// de datos (C en adelante) — a veces terminan con formato Fecha (arrastrado
// de quién sabe dónde), y ahí un valor como cash=0 o agendas=9 escrito con
// USER_ENTERED se guarda y se muestra como "30/12/1899" o "09/01/1900" en
// vez del número. Pasó en VALERIA/FRANCO/PAULA el 04/09/2026 y rompió el
// "Cash Collected" del dashboard (leía el string de la fecha como si fuera
// plata). Forzar acá el formato a NUMBER apenas se crea la fila evita que
// vuelva a pasar, sea cual sea el motivo por el que la fila nueva queda con
// formato Fecha.
async function limpiarFormatoNumericoFilaNueva(sheets, sheetName, rowIndex, fechaCol) {
  const sheetId = await getSheetIdByName(sheets, sheetName);
  if (sheetId === null) return;
  const fechaColIndex = fechaCol.charCodeAt(0) - "A".charCodeAt(0);
      ...
      requests: [{
        repeatCell: {
          range: {
            ...
            startColumnIndex: fechaColIndex + 1,
            endColumnIndex: fechaColIndex + 30
          },
...
async function asegurarHeaderVentasIndividual(sheets, q) {
  ...
  // Dos bloques chequeados POR SEPARADO — una hoja vieja ya tiene I:K
  // puesto (cash orgánico/mixto/ventas) pero nunca tuvo L:U (estrellas).
  // Chequear un solo rango de punta a punta (mirando solo la primera
  // celda) hacía que, al ya estar I4 puesto, se diera por hecho que TODO
  // el rango — incluido L:U, agregado después — ya estaba, y nunca se
  // llegaba a escribir.
  const rangeVentas = `${q}!${COL_LETTERS.cashOrganico}${HEADER_ROW}:${COL_LETTERS.ventas}${HEADER_ROW}`;
  const rangeEstrellas = `${q}!${COL_LETTERS.agendas1}${HEADER_ROW}:${COL_LETTERS.ventas5}${HEADER_ROW}`;

  const [curVentas, curEstrellas] = await Promise.all([
    sheets.spreadsheets.values.get({ spreadsheetId: SPREADSHEET_ID, range: rangeVentas }),
    sheets.spreadsheets.values.get({ spreadsheetId: SPREADSHEET_ID, range: rangeEstrellas })
  ]);

  const ops = [];
  if (!(curVentas.data.values && curVentas.data.values[0] && curVentas.data.values[0][0])) {
    ops.push(sheets.spreadsheets.values.update({
      spreadsheetId: SPREADSHEET_ID,
      range: rangeVentas,
      valueInputOption: "USER_ENTERED",
  ...
  if (!(curEstrellas.data.values && curEstrellas.data.values[0] && curEstrellas.data.values[0][0])) {
    ops.push(sheets.spreadsheets.values.update({
      spreadsheetId: SPREADSHEET_ID,
      range: rangeEstrellas,
      valueInputOption: "USER_ENTERED",
      requestBody: {
        values: [[
          "AGENDAS 1★", "AGENDAS 2★", "AGENDAS 3★", "AGENDAS 4★", "AGENDAS 5★",
          "VENTAS 1★", "VENTAS 2★", "VENTAS 3★", "VENTAS 4★", "VENTAS 5★"
        ]]
      }
    }));
  }
...
async function escribirTotalesDelDia(sheets, setter, fecha, info) {
  ...
  const agendasEstrellas = info.agendasEstrellas || {};
  const ventasEstrellas = info.ventasEstrellas || {};
  const data = [
    ...
    { range: `${q}!${COL_LETTERS.agendas1}${rowIndex}`, values: [[agendasEstrellas[1] || 0]] },
    { range: `${q}!${COL_LETTERS.agendas2}${rowIndex}`, values: [[agendasEstrellas[2] || 0]] },
    { range: `${q}!${COL_LETTERS.agendas3}${rowIndex}`, values: [[agendasEstrellas[3] || 0]] },
    { range: `${q}!${COL_LETTERS.agendas4}${rowIndex}`, values: [[agendasEstrellas[4] || 0]] },
    { range: `${q}!${COL_LETTERS.agendas5}${rowIndex}`, values: [[agendasEstrellas[5] || 0]] },
    { range: `${q}!${COL_LETTERS.ventas1}${rowIndex}`, values: [[ventasEstrellas[1] || 0]] },
    { range: `${q}!${COL_LETTERS.ventas2}${rowIndex}`, values: [[ventasEstrellas[2] || 0]] },
    { range: `${q}!${COL_LETTERS.ventas3}${rowIndex}`, values: [[ventasEstrellas[3] || 0]] },
    { range: `${q}!${COL_LETTERS.ventas4}${rowIndex}`, values: [[ventasEstrellas[4] || 0]] },
    { range: `${q}!${COL_LETTERS.ventas5}${rowIndex}`, values: [[ventasEstrellas[5] || 0]] }
  ];
  ...
  // El resync recalcula el día ENTERO cada 15 minutos, no solo la primera
  // vez — si el formato de la fila se corrompe a Fecha en algún momento
  // (ver limpiarFormatoNumericoFilaNueva), cada reescritura posterior lo
  // volvía a corromper porque findOrCreateDateRow solo limpia el formato
  // cuando CREA la fila, no cuando la encuentra ya existente. Se repite
  // acá para que cada corrida deje el formato bien sin importar cómo haya
  // quedado antes.
  await limpiarFormatoNumericoFilaNueva(sheets, sheetName, rowIndex, COL_LETTERS.fecha);
