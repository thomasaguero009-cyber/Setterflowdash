...
async function hyrosFetchDataForDate(apiKey, fecha, fechaHasta) {
  ...
  for (const f of dias) {
    ...
    let debugSaleMuestra = 0;
    await hyrosForEachSale(apiKey, fromDate, toDate, (sale) => {
      if (debugSaleMuestra < 8) {
        debugSaleMuestra++;
        console.log("DEBUG_SALE_RAW:", JSON.stringify(sale));
      }
    ...
    let debugCallMuestra = 0;
    await hyrosForEachCall(apiKey, fromDate, toDate, (call) => {
      if (debugCallMuestra < 8) {
        debugCallMuestra++;
        console.log("DEBUG_CALL_RAW:", JSON.stringify(call));
      }
