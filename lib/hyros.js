...
async function hyrosFetchDataForDate(apiKey, fecha, fechaHasta) {
  ...
  const data = Object.keys(HYROS_SETTER_CODES).reduce((acc, setter) => {
    acc[setter] = { cash: 0, leads: 0, agendas: 0, ventas: 0, cashOrganico: 0, cashMixto: 0 };
    return acc;
  }, {});
  const fuentes = {};
  ...
  for (const f of dias) {
    ...
    await hyrosForEachLead(apiKey, fromDate, toDate, (lead) => {
      ...
      // DEBUG TEMPORAL — sacar apenas se entienda por qué "paula" (st02)
      // queda en cero mientras los demás setters sí suman leads hoy.
      if (!setters.length && (lead.tags || []).some((t) => /st02|_02\b/i.test(String(t)))) {
        console.log("DEBUG_ST02_SIN_MATCH:", JSON.stringify(lead.tags));
      }
