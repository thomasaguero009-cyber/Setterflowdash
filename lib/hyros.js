...
async function hyrosFetchDataForDate(apiKey, fecha, fechaHasta) {
  ...
  for (const f of dias) {
    ...
    // "Agendas" = llamadas ("calls") agendadas en Hyros. Cada una trae su
    // propio tag de calidad ("call.tag": "$highlevel-udc-N-estrellas...").
    await hyrosForEachCall(apiKey, fromDate, toDate, (call) => {
      const leadTags = call.lead && call.lead.tags;
      const setters = hyrosSetterForTags(leadTags);
      // Si Hyros no le puso ninguna etiqueta de estrellas a esta agenda
      // (pasa seguido), cuenta como 1 estrella — la calidad más baja, no
      // se descarta del desglose.
      const estrellaAgenda = hyrosExtraerEstrellas(call.tag) || hyrosExtraerEstrellas(leadTags) || 1;
      setters.forEach((setter) => {
